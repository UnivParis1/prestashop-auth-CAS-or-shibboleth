	...
	public function process()
	{
		if (Tools::isSubmit('email'))
		{
			...
			else
			{
				...
				//test du groupe d'appatenance
				elseif ($customer->id_default_group > 1)
					$this->errors[] = Tools::displayError('You can regenerate your password only on SSO interface');
				else
				{
		...
		elseif (($token = Tools::getValue('token')) && ($id_customer = (int)(Tools::getValue('id_customer'))))
		{
			...
			if ($email)
			{
				...
				//test du groupe d'appatenance
				elseif ($customer->id_default_group > 1)
					$this->errors[] = Tools::displayError('You can regenerate your password only on CAS interface');
				else
				{
					if ((strtotime($customer->last_passwd_gen.'+'.(int)($min_time = Configuration::get('PS_PASSWD_TIME_FRONT')).' minutes') - time()) > 0)
