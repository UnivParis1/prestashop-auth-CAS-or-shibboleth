...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$("fieldset").children('p').each(function() {
			if(group > 1) {
				$(this).children('select,input').attr('disabled','disabled');
				$(this).children('input#old_passwd,input#passwd,input#confirmation,input.button').parent().hide();
			}
			else {
				$(this).children('select,input').removeAttr('disabled');
				$(this).children('input#old_passwd,input#passwd,input#confirmation,input.button').parent().show();
			}
...
{else}
	<h3>{if $cookie->group > 1}{l s='You have no permission to update your personal information if it has changed. Please contact your administrator'}{else}{l s='Please do not hesitate to update your personal information if it has changed.'}{/if}</h3>
