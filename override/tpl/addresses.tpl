...
<div class="addresses">
	...
	{foreach from=$multipleAddresses item=address name=myLoop}
		<ul class="address {if $smarty.foreach.myLoop.last}last_item{elseif $smarty.foreach.myLoop.first}first_item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{else}item{/if}">
			...
			{/foreach}
			<li class="address_update"> {if $smarty.foreach.myLoop.first AND $cookie->group > 1}{else}
			<a href="{$link->getPageLink('address.php', true)}?id_address={$address.object.id|intval}" title="{l s='Update'}">{l s='Update'}</a>{/if}</li>
			<li class="address_delete">{if $smarty.foreach.myLoop.first AND $cookie->group > 1}{else}<a href="{$link->getPageLink('address.php', true)}?id_address={$address.object.id|intval}&amp;delete" onclick="return confirm('{l s='Are you sure?'}');" title="{l s='Delete'}">{l s='Delete'}</a>{/if}</li>
		</ul>
	{/foreach}
	<p class="clear" />
</div>
