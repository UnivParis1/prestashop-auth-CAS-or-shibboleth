	...
	$(document).ready(function() {
		if (firstAddress == 1) {
			$("fieldset").children('p').each(function() {
				if (group > 1) {
					$(this).children('select,input,textarea').attr('disabled','disabled');
					$(this).children('input.button').parent().hide();
				}
				else {
					$(this).children('select,input,textarea').removeAttr('disabled');
					$(this).children('input.button').parent().show();
				}
			});	
			$('p').children('input.button').hide();
		}
	});
</script>
{/if}
